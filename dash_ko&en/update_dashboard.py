...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "domestic_title": "유진증권 안상현 센터장의 국장 마감 대쉬보드",
    "overseas_title": "유진증권 안상현 센터장의 미장 마감 대쉬보드",
    "brand": "",
    "data_source": "live",
...
def render_hero_title(title: str, config: dict[str, Any]) -> str:
    raw_title = str(config.get("brand") or title)
    prefix = "유진증권 안상현 센터장의 "
    if raw_title.startswith(prefix):
        return (
            f'<div class="brand-kicker">{esc(prefix.strip())}</div>'
            f'<div class="brand-main">{esc(raw_title[len(prefix):])}</div>'
        )
    return f'<div class="brand-main">{esc(raw_title)}</div>'
...
def build_html(theme: str, title: str, payload: dict[str, Any], config: dict[str, Any]) -> str:
    ...
    hero_title = render_hero_title(title, config)
    headline = str(payload.get("headline", "")).strip()
    hero_badge = f'<div class="badge">{esc(headline)}</div>' if headline else ""
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    body {{
      ...
      background: var(--bg);
      color: var(--text);
      font-family: "Pretendard", "Noto Sans KR", "SUIT", "Apple SD Gothic Neo", "Malgun Gothic", "Segoe UI", sans-serif;
      letter-spacing: 0;
    }}
    main {{
    ...
    .hero {{
      ...
      border-radius: 8px;
      background: linear-gradient(135deg, #173a78 0%, #265b9f 56%, #417899 100%);
      color: white;
    ...
    @media (max-width: 900px) {{
      ...
      .brand-main {{ font-size: clamp(28px, 8vw, 38px); }}
...
<body class="{theme}">
  <main>
    <header class="hero">
      <div>
        <div class="brand">{hero_title}</div>
        <div class="subtitle">{esc(payload['as_of_date'])} · 자동 업데이트</div>
      </div>
      <div class="hero-meta">
