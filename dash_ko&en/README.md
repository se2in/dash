...
국내 대시보드의 `섹터 온도계`는 `naver_issues.py`의 네이버 뉴스 수집/테마 점수를 재사용합니다.

- 네이버 금융 뉴스 제목과 요약에서 섹터별 키워드 점수를 계산합니다.
- 섹터별 대표 종목군을 네이버금융에서 조회합니다.
- 등락률 상위 3개 종목을 카드에 표시합니다.
- 섹터 카드를 클릭하면 해당 섹터의 대표 관련 뉴스로 이동합니다.

섹터별 대표 종목군은 `sector_temperature.py`의 `DEFAULT_SECTOR_STOCKS`에서 관리합니다.
